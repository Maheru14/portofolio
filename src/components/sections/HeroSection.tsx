...
export const HeroSection = ({ profile }: HeroProps) => {
  ...
  return (
    <section id="profile" className="relative left-1/2 right-1/2 -mx-[50vw] w-screen overflow-hidden bg-gradient-to-b from-white via-[#A5E3E9] to-white">
      ...
      <div className={`absolute top-[15%] left-[10%] w-[40rem] h-[40rem] ${canRenderHeavyEffects ? 'bg-[#50E8F4]/40 rounded-full bg-blob-heavy mix-blend-multiply animate-pulse' : ''}`} style={canRenderHeavyEffects ? { animationDuration: '6s' } : { background: 'radial-gradient(circle, rgba(80,232,244,0.4) 0%, transparent 60%)' }} />
      <div className={`absolute bottom-[20%] right-[5%] w-[45rem] h-[45rem] ${canRenderHeavyEffects ? 'bg-[#34D399]/30 rounded-full bg-blob-heavy mix-blend-multiply animate-pulse' : ''}`} style={canRenderHeavyEffects ? { animationDuration: '8s' } : { background: 'radial-gradient(circle, rgba(52,211,153,0.3) 0%, transparent 60%)' }} />
      <div className={`absolute top-[30%] left-[40%] w-[35rem] h-[35rem] ${canRenderHeavyEffects ? 'bg-[#818CF8]/20 rounded-full bg-blob-heavy mix-blend-multiply animate-pulse' : ''}`} style={canRenderHeavyEffects ? { animationDuration: '10s' } : { background: 'radial-gradient(circle, rgba(129,140,248,0.2) 0%, transparent 60%)' }} />
      ...
      <div className={`absolute top-[20%] left-[10%] w-72 h-72 opacity-70 ${canRenderHeavyEffects ? 'bg-[#50E8F4]/30 rounded-full bg-blob-heavy mix-blend-multiply animate-pulse' : ''}`} style={!canRenderHeavyEffects ? { background: 'radial-gradient(circle, rgba(80,232,244,0.3) 0%, transparent 70%)' } : undefined} />
      <div className={`absolute bottom-[20%] right-[10%] w-96 h-96 opacity-70 ${canRenderHeavyEffects ? 'bg-[#99E1D9]/30 rounded-full bg-blob-heavy mix-blend-multiply' : ''}`} style={!canRenderHeavyEffects ? { background: 'radial-gradient(circle, rgba(153,225,217,0.3) 0%, transparent 70%)' } : undefined} />
      ...
      <ContainerScroll>
        <div className="flex flex-col lg:flex-row items-center justify-between w-full max-w-[1400px] mx-auto gap-16 lg:gap-12 bg-white/70 transition-all duration-300 glass-effect border border-white/20 p-8 lg:p-16 rounded-[3rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.4),0_8px_32px_rgba(0,0,0,0.05)] relative z-10">
          ...
          >
            ...
            {/* Main Photo Card */}
            <motion.div 
              animate={isMobile ? {} : (canRenderHeavyEffects ? { y: [0, -15, 0] } : {})}
              transition={canRenderHeavyEffects ? { repeat: Infinity, duration: 6, ease: "easeInOut" } : undefined}
